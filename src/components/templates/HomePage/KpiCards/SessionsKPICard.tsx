    ...
    return (
        <div className="neu__norm p-5 rounded-3xl flex justify-between items-center hover:scale-[1.05] transition-transform duration-700">
            <div className="font-semibold flex gap-1">
                <MdEventAvailable className="lg:w-14 lg:h-14 w-12 h-12 text-pink-500" />
                <div>
                    <h3 className="text-pink-500">{t('mainDashboard.appointments')}</h3>
                    <div className="text-(--alt-text) text-[8px] lg:text-[10px]">{t('mainDashboard.today')}</div>
                    <div className="text-(--alt-text) text-[8px] lg:text-[10px]"><span className="text-(--greens)">9 {t('mainDashboard.completed')}</span> - <span className="text-(--reds)">1 {t('mainDashboard.canceled')}</span></div>
                </div>
            </div>
